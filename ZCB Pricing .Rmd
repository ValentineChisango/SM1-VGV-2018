...
Antithetic_variates = function(alpha =0.08,beta=0.8,sigma=0.005,T=5,r = 0.08,n=10000,s=1250){
  ...
  for (i in 1:n){
    z = rnorm(1)# Generate standard normal values
    z.=-z
    for (j in 2:(s+1)){
      dr = (alpha-beta*sim_path[j-1,i])*dt+sigma*sqrt(dt)*z # Same Euler Discretization as in the Crude Monte Carlo function
      dr_i = (alpha-beta*sim_path_inv[j-1,i])*dt + sigma*sqrt(dt)*z. # Euler Discretization for the use of -Z_i
      sim_path[j,i] =sim_path[j-1,i]+dr
