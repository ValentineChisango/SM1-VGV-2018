...
Crude_Monte_Carlo=function(alpha =0.08,beta=0.8,sigma=0.005,T=5,r = 0.08,n=1000,s=1250){
  ...
  z=rnorm(n)# Generation of standard normal random variables
  
  for (i in 1:n){
    for (j in 2:(s+1)){
      dr=(alpha-beta*sim_path[j-1,i])*dt+sigma*sqrt(dt)*z[i]# Discretize the SDE of the interest model using Euler Discretization to simulate rates over discrete sub-intervals
...
Antithetic_variates = function(alpha =0.08,beta=0.8,sigma=0.005,T=5,r = 0.08,n=1000,s=1250){
  ...
  for (i in 1:n){
    for (j in 2:(s+1)){
      ...
      dr_i = (alpha-beta*sim_path_inv[j-1,i])*dt + sigma*sqrt(dt)*z.[i] # Euler Discretization for the use of -Z_i
      sim_path[j,i] =sim_path[j-1,i]+dr
...
Moment_Matching = function(alpha = 0.08, beta = 0.8, sigma = 0.005, T = 5,r = 0.08, n = 1000, s = 1250){
  ...
  sim_path[1,]= r
  z=rnorm(n)
  z_m= z-mean(z) # Moment matching transformation of Z_i to match finite moments 
  
  # Follow the same process as in the Monte Carlo approach , but use the transformed Z_i's to simulate the rates
  for (i in 1:n){
    for (j in 2:(s+1)){
      dr=(alpha-beta*sim_path[j-1,i])*dt+sigma*sqrt(dt)*z_m[i] 
      sim_path[j,i]=sim_path[j-1,i]+dr
    }
  }
...
# Make use of the CIR model as the control variate process.
...
CIR_MC= function(alpha =0.08,beta=0.8,sigma=0.005,T=5,r = 0.08,n=1000,s=1250){
  
  dt <- T/s # Length of sub-intervals
  sim_path= matrix(0,s+1,n) # Matrix that stores rate pathways
  sim_path[1,]= r
  z=rnorm(n) # Generation of standard normal variables
  
  for (i in 1:n){
    for (j in 2:(s+1)){
      dr=(alpha-beta*sim_path[j-1,i])*dt+(sigma*sqrt(dt)*z[i]*sqrt(sim_path[j-1,i]))# Euler Discretization
      sim_path[j,i]=sim_path[j-1,i]+dr
...
# The control variate estimate is taken as the difference between the the Crude MOnte carlo estimates of the Vasicek and CIR model plus the closed form of the CIR model

Control_Variates_estimate= function(alpha =0.08,beta=0.8,sigma=0.005,t=0,T=5,r = 0.08,n=1000,s=1250){
...
plot(x=1:8,y=senses[1,2:9], ylim =c(0.58,0.7), xlab = "Numerical Method", ylab = "Bond Price", main = "Sensitivity Test: Alpha & Beta",  xaxt = "n", col ="black", pch = 19, font.main = 1)
axis(1, at=1:8, labels=xvec)
...
var(CR) # variance of crude monte carlo
var(AN) # variance of antithetic variates
var(MM) # variance of moment matching
var(CV) # variance of control variates
...
trials_vec <- seq(10,2000,50) # Vector holding time steps
...
# Plot of convergence
plot(closed_1, x = trials_vec, type = "l",ylim = c(0.57,0.70), xlab = "Number of Trials", ylab = "Estimated Price", lty = 2, lwd = 2,font.main = 1, main = "Monte Carlo Bond Prices over Number of Trials")
