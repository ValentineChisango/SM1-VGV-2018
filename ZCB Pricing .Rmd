...
Crude_Monte_Carlo=function(alpha =0.08,beta=0.8,sigma=0.005,T=5,r = 0.08,n=10000,s=1250){
  ...
  for (i in 1:n){
    for (j in 2:(s+1)){
      dr=(alpha-beta*sim_path[j-1,i])*dt+sigma*sqrt(dt)*z[j]# Discretize the SDE of the interest model using Euler Discretization to simulate rates over discrete sub-intervals
...
Antithetic_variates = function(alpha =0.08,beta=0.8,sigma=0.005,T=5,r = 0.08,n=10000,s=1250){
  ...
  for (i in 1:n){
    ...
    for (j in 2:(s+1)){
      dr = (alpha-beta*sim_path[j-1,i])*dt+sigma*sqrt(dt)*z[j] # Same Euler Discretization as in the Crude Monte Carlo function
      dr_i = (alpha-beta*sim_path_inv[j-1,i])*dt + sigma*sqrt(dt)*z.[j] # Euler Discretization for the use of -Z_i
      sim_path[j,i] =sim_path[j-1,i]+dr
...
Moment_Matching = function(alpha = 0.08, beta = 0.8, sigma = 0.005, T = 5,r = 0.08, n = 10000, s = 1250){
  ...
  for (i in 1:n){
    for (j in 2:(s+1)){
      dr=(alpha-beta*sim_path[j-1,i])*dt+sigma*sqrt(dt)*z_m[j] 
      sim_path[j,i]=sim_path[j-1,i]+dr
    }
  }
...
CIR_MC= function(alpha =0.08,beta=0.8,sigma=0.005,T=5,r = 0.08,n=10000,s=1250){
  ...
  for (i in 1:n){
    for (j in 2:(s+1)){
      dr=(alpha-beta*sim_path[j-1,i])*dt+(sigma*sqrt(dt)*z[j]*sqrt(sim_path[j-1,i]))# Euler Discretization
      sim_path[j,i]=sim_path[j-1,i]+dr
